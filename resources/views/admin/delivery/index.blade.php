        ...
        <form id="formCrud">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="formCrudModalTitle">Nuevo Coste</h5>
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label for="name" class="col-form-label">Kilometros: <small>(<span
                                    class="text-danger">*</span>)</small></label>
                        <input type="text" class="form-control" id="kilometros" name="kilometros" autofocus required>
                    </div>
                    <div class="form-group">
                        <label for="name" class="col-form-label">Importe: <small>(<span
                                    class="text-danger">*</span>)</small></label>
...
<script>
    $( document ).ready(function() {
        var data = {
            ...
            columns: [
                {data: 'id', name: 'id'},
                {data: 'kilometros', name: 'Kilometros'},
    ...
    $('#formCrud').on('submit', function(e){
        ...
        if( $('#id').val()  ){
            ...
            var ruta = "{{ route('delivery.update') }}";
            var formData = new FormData($('#formCrud')[0]);
            update(formData, ruta);
        }else{
            console.log('store');
            var formData = new FormData($('#formCrud')[0]);
            var ruta = "{{ route('delivery.store') }}";
            store(formData, ruta);
        }
        console.log(ruta);
    });
