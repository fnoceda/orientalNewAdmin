...
function filtrar(){
        var _newUrl = "{{ route('admin.reportes.ventas.data') }}"  + '?desde='+$('#fecha_desde').val()+'&hasta='+$('#fecha_hasta').val()+'&cliente_id='+$('#cliente_id').val()+'&estado='+$('#estado').val();
...
function getListado(){
    $('#esperando').show()
        if ( $.fn.dataTable.isDataTable( '#tablaListado' ) ) { 
            ...
            dom: 'Bfrtip',
            'ajax': {
                url: "{{ route('admin.reportes.ventas.data') }}",
                    data : function(d){
